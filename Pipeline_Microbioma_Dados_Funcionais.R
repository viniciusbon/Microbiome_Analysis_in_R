...
do_pca <- function(ds, dsn) {
  ...
  grp_n<-table(pdf2$Group); edat<-pdf2[pdf2$Group %in% names(grp_n[grp_n>=4]),]
  ...
  if(nrow(edat)>=4) p12<-p12+stat_ellipse(data=edat,aes(group=Group),type="norm",linetype="dashed",alpha=.5)
  ...
  if(np>=3){
    p13<-ggplot(pdf2,aes(PC1,PC3,color=Group,label=Sample_ID))+geom_point(size=3.5,alpha=.85)+
      ...
      theme_minimal(base_size=13)+theme(plot.title=element_text(face="bold"))
    if(nrow(edat)>=4) p13<-p13+stat_ellipse(data=edat,aes(group=Group),type="norm",linetype="dashed",alpha=.5)
  }
...
alpha_results_all<-list(); alpha_stats_all<-list(); alpha_plots_all<-list()
rarefaction_info_all<-list(); data_type_info<-list()
...
for(ds in names(datasets_grouped_list)){
  ...
  cat(sprintf("  Tipo:%s | Matriz:%dx%d | Depth:%d\n",dtype$type,nrow(cm),ncol(cm),round(rd)))
  
  rarefaction_info_all[[ds]]<-data.frame(Dataset=ds,Type=dtype$type,Min=round(rd),Max=round(max(sd2)),
                                         Med=round(median(sd2)),Samples=nrow(cm),Features=ncol(cm),stringsAsFactors=FALSE)
  ...
  po<-pam("Observed","Richness"); pc<-pam("Chao1","Chao1")
  ps<-pam("Shannon","Shannon"); pi2<-pam("Simpson","Simpson")
...
for(ds in names(datasets_grouped_list)){
  ...
  cat(paste("  Matriz:",nrow(cr),"x",ncol(cr),"|",bl,"\n"))
  ...
  grp_n_beta<-table(mb$Group); eok<-all(grp_n_beta>=4)
  pp_list<-list()
  for(dn in names(dl2)){
    ...
    p<-ggplot(pd3,aes(A1,A2,color=Group,label=Sample))+geom_point(size=3.5,alpha=.85)+
      geom_text_repel(size=2.5,max.overlaps=25,show.legend=FALSE)+scale_color_manual(values=group_palette)+
      annotate("label",x=Inf,y=-Inf,label=sbd[[dn]]$annot_text,hjust=1.03,vjust=-.15,size=2.8,fill="white",alpha=.88,family="mono")+
      labs(title=sprintf("PCoA-%s-%s",ds,dn),subtitle=bl,x=sprintf("Axis1(%.1f%%)",vp[1]),y=sprintf("Axis2(%.1f%%)",vp[2]))+
      theme_bw(base_size=13)+theme(plot.title=element_text(face="bold"))
    if(eok) p<-p+stat_ellipse(aes(group=Group),type="norm",linetype="dashed",alpha=.6)
  ...
  for(dn in names(dl2)){set.seed(42)
    ...
    p<-ggplot(ns2,aes(NMDS1,NMDS2,color=Group,label=Sample))+geom_point(size=3.5,alpha=.85)+
      ...
      labs(title=sprintf("NMDS-%s-%s",ds,dn),x="NMDS1",y="NMDS2")+theme_bw(base_size=13)+theme(plot.title=element_text(face="bold"))
    if(eok) p<-p+stat_ellipse(aes(group=Group),type="norm",linetype="dashed",alpha=.6)
  ...
  ht2<-Heatmap(hz,name="Z",col=cf2,top_annotation=ca2,cluster_rows=hcr,cluster_columns=hcc,
               ...
               row_names_gp=gpar(fontsize=rfs),row_names_side="left",column_title=sprintf("%s-Top%d",ds,nth),border=TRUE)
...
# ═══════════════════════════════════════════════════════════════════════════════
# FASE 11 — MAASLIN2 + VOLCANO (q-value E p-value)
# ═══════════════════════════════════════════════════════════════════════════════
cat("\n",rep("=",70),"\n=== FASE 11: MAASLIN2 ===\n",rep("=",70),"\n\n")
...
for(ds in names(datasets_grouped_list)){
  ...
  n_sig_p<-sum(mr$pval<.05,na.rm=TRUE)
  nec_p<-sum(mr$pval<.05 & mr$coef>0,na.rm=TRUE)
  ner_p<-sum(mr$pval<.05 & mr$coef<0,na.rm=TRUE)
  cat(sprintf("  q<.25:%d (up:%d dn:%d) | p<.05:%d (up:%d dn:%d)\n",n25,nec,ner,n_sig_p,nec_p,ner_p))
  ...
  # Labels p
  sf_p<-mr[mr$pval<0.05,]
  tl_p<-if(nrow(sf_p)>0) do.call(rbind,lapply(unique(sf_p$Direction_pval),function(d){
    s2<-sf_p[sf_p$Direction_pval==d,];s2<-s2[order(s2$pval,-abs(s2$coef)),];head(s2,12)})) else NULL
  
  # ── VOLCANO q-value ──
  pv_q<-ggplot(mr,aes(coef,neg_log10_q,color=Direction,size=abs(coef)))+geom_point(alpha=.72)+
    ...
    labs(title=paste("q-value (FDR) -",ds),subtitle=sprintf("%s vs %s | %s | BH | q<%.2f",comparison_group,ref_group,nl,q_threshold),
         x=sprintf("Coefficient\n<- %s | %s ->",ref_group,comparison_group),y=expression(-log[10](q)),color=NULL)+
    theme_bw(base_size=12)+theme(plot.title=element_text(face="bold",size=13),plot.subtitle=element_text(color="grey40",size=8.5),legend.position="bottom")
  if(!is.null(tl)&&nrow(tl)>0) pv_q<-pv_q+geom_text_repel(data=tl,aes(label=feature_clean),size=2.7,max.overlaps=30,segment.color="grey50",box.padding=.45,show.legend=FALSE)
  if(nrow(mr)>0) pv_q<-pv_q+
    annotate("text",x=min(mr$coef,na.rm=TRUE),y=max(mr$neg_log10_q,na.rm=TRUE),label=sprintf("n=%d",ner),hjust=0,vjust=1,size=3.8,color=group_palette[ref_group],fontface="bold")+
    annotate("text",x=max(mr$coef,na.rm=TRUE),y=max(mr$neg_log10_q,na.rm=TRUE),label=sprintf("n=%d",nec),hjust=1,vjust=1,size=3.8,color=group_palette[comparison_group],fontface="bold")+
    annotate("text",x=max(mr$coef,na.rm=TRUE),y=-log10(q_threshold),label=sprintf("q=%.2f",q_threshold),hjust=1,vjust=-.4,size=3,color="grey35",fontface="italic")
  
  # ── VOLCANO p-value ──
  pv_p<-ggplot(mr,aes(coef,neg_log10_p,color=Direction_pval,size=abs(coef)))+geom_point(alpha=.72)+
    geom_hline(yintercept=-log10(0.05),linetype="dashed",color="grey35")+
    geom_vline(xintercept=c(-coef_threshold,coef_threshold),linetype="dashed",color="grey35")+
    scale_color_manual(values=volcano_colors)+scale_size_continuous(range=c(1.5,4.5),guide="none")+
    labs(title=paste("p-value (uncorrected) -",ds),subtitle=sprintf("%s vs %s | %s | p<0.05",comparison_group,ref_group,nl),
         x=sprintf("Coefficient\n<- %s | %s ->",ref_group,comparison_group),y=expression(-log[10](p)),color=NULL)+
    theme_bw(base_size=12)+theme(plot.title=element_text(face="bold",size=13),plot.subtitle=element_text(color="grey40",size=8.5),legend.position="bottom")
  if(!is.null(tl_p)&&nrow(tl_p)>0) pv_p<-pv_p+geom_text_repel(data=tl_p,aes(label=feature_clean),size=2.7,max.overlaps=30,segment.color="grey50",box.padding=.45,show.legend=FALSE)
  if(nrow(mr)>0) pv_p<-pv_p+
    annotate("text",x=min(mr$coef,na.rm=TRUE),y=max(mr$neg_log10_p,na.rm=TRUE),label=sprintf("n=%d",ner_p),hjust=0,vjust=1,size=3.8,color=group_palette[ref_group],fontface="bold")+
    annotate("text",x=max(mr$coef,na.rm=TRUE),y=max(mr$neg_log10_p,na.rm=TRUE),label=sprintf("n=%d",nec_p),hjust=1,vjust=1,size=3.8,color=group_palette[comparison_group],fontface="bold")+
    annotate("text",x=max(mr$coef,na.rm=TRUE),y=-log10(0.05),label="p=0.05",hjust=1,vjust=-.4,size=3,color="grey35",fontface="italic")
  
  # ── DUAL PANEL: q + p lado a lado ──
  pv_dual<-pv_q+pv_p+plot_layout(guides="collect")+
    plot_annotation(title=sprintf("DA - %s - MaAsLin2",ds),
                    subtitle=sprintf("%s vs %s | q<%.2f: %d | p<0.05: %d",comparison_group,ref_group,q_threshold,n25,n_sig_p),
                    theme=theme(plot.title=element_text(face="bold",size=15),plot.subtitle=element_text(color="grey40",size=10)))&
  ...
  if(nrow(tb)>0){
    ...
    print(pb)
    ppnl<-pv_q+pb+plot_layout(widths=c(1.4,1))+plot_annotation(title=paste("DA -",ds)); print(ppnl)
  }
  ...
  dex<-data.frame(Dataset=ds,Feature=mr$feature_clean,Coefficient=round(mr$coef,4),
                  ...
                  Direction_q=mr$Direction,Direction_p=mr$Direction_pval,Normalization=nl,stringsAsFactors=FALSE)
  if("N" %in% names(mr)) dex$N<-mr$N
  if("N.not.0" %in% names(mr)) dex$N_not_zero<-mr$N.not.0
  
  da_results_all[[ds]]<-dex
  da_stats_all[[ds]]<-data.frame(Dataset=ds,Tested=nrow(mr),Sig_q025=n25,Sig_q005=n05q,
                                 Sig_p005=n_sig_p,Enr_comp_q=nec,Enr_ref_q=ner,Enr_comp_p=nec_p,Enr_ref_p=ner_p,
                                 Normalization=nl,stringsAsFactors=FALSE)
  da_plots_all[[ds]]<-list(volcano_q=pv_q,volcano_p=pv_p,volcano_dual=pv_dual,bar=pb,panel=ppnl)
  cat(paste("\n  OK:",ds,"\n\n"))
}
...
cat("\n=== RESUMO DA ===\n"); print(all_da_stats)
# ═══════════════════════════════════════════════════════════════════════════════
# FASE 12 — EXPORT UNIFICADO COMPLETO
# ═══════════════════════════════════════════════════════════════════════════════
cat("\n",rep("=",70),"\n=== FASE 12: EXPORT ===\n",rep("=",70),"\n\n")
...
add_fmt_sheet<-function(wb,sn,data,hdr_color="#4472C4"){
  ...
  hs<-createStyle(fontColour="#FFFFFF",fgFill=hdr_color,fontName="Calibri",fontSize=11,
                  textDecoration="bold",halign="center",valign="center",border="TopBottomLeftRight",borderColour="#FFFFFF",wrapText=TRUE)
  ...
  writeData(wb,sn,data,headerStyle=hs); nr<-nrow(data); nc<-ncol(data)
...
safe_export<-function(expr,label){
  tryCatch(expr,error=function(e){cat(sprintf("    ERRO %s: %s\n",label,e$message));ec$errors<<-ec$errors+1})
}
...
for(ds in names(datasets_grouped_list)){
  ...
  if(!is.null(pca_results_grouped[[ds]])){
    ...
    if(!is.null(pr$plot_pc12)) safe_export({save_gg(pr$plot_pc12,file.path(dp,"pca_PC1_vs_PC2"),9,7);ec$plots<-ec$plots+1},"pca12")
    if(!is.null(pr$plot_pc13)) safe_export({save_gg(pr$plot_pc13,file.path(dp,"pca_PC1_vs_PC3"),9,7);ec$plots<-ec$plots+1},"pca13")
  }
  ...
  if(!is.null(alpha_plots_all[[ds]])){
    ...
    for(it in list(list(p=ap$depth,n="sequencing_depth",w=13,h=6),list(p=ap$facet,n="alpha_all_metrics",w=12,h=10),
                   list(p=ap$panel,n="alpha_2x2_panel",w=14,h=12),list(p=ap$observed,n="alpha_observed",w=7,h=6),
                   list(p=ap$chao1,n="alpha_chao1",w=7,h=6),list(p=ap$shannon,n="alpha_shannon",w=7,h=6),
                   list(p=ap$simpson,n="alpha_simpson",w=7,h=6)))
      if(!is.null(it$p)) safe_export({save_gg(it$p,file.path(da,it$n),it$w,it$h);ec$plots<-ec$plots+1},it$n)
  }
  ...
  if(!is.null(beta_plots_all[[ds]])){
    ...
    if(!is.null(bp$pcoa_plots)) for(dn in names(bp$pcoa_plots)){fn<-gsub("[^a-zA-Z0-9]","_",dn)
    safe_export({save_gg(bp$pcoa_plots[[dn]],file.path(db,paste0("pcoa_",fn)),9,7);ec$plots<-ec$plots+1},paste0("pcoa_",fn))}
    if(!is.null(bp$pcoa_panel)) safe_export({save_gg(bp$pcoa_panel,file.path(db,"pcoa_panel"),16,7);ec$plots<-ec$plots+1},"pcoa_panel")
    if(!is.null(bp$nmds_plots)&&length(bp$nmds_plots)>0) for(dn in names(bp$nmds_plots)){fn<-gsub("[^a-zA-Z0-9]","_",dn)
    safe_export({save_gg(bp$nmds_plots[[dn]],file.path(db,paste0("nmds_",fn)),9,7);ec$plots<-ec$plots+1},paste0("nmds_",fn))}
    if(!is.null(bp$nmds_panel)) safe_export({save_gg(bp$nmds_panel,file.path(db,"nmds_panel"),16,7);ec$plots<-ec$plots+1},"nmds_panel")
    if(!is.null(bp$heatmap)) safe_export({
      png(file.path(db,"heatmap_top_features.png"),width=14,height=12,units="in",res=300,bg="white")
      draw(bp$heatmap,merge_legend=TRUE); dev.off(); cat("    Salvo: heatmap_top_features.png\n"); ec$plots<-ec$plots+1
    },"heatmap")
  }
  ...
  if(!is.null(da_plots_all[[ds]])){
    dap<-da_plots_all[[ds]]
    for(it in list(list(p=dap$volcano_q,n="volcano_qvalue",w=11,h=8),
                   list(p=dap$volcano_p,n="volcano_pvalue",w=11,h=8),
                   list(p=dap$volcano_dual,n="volcano_dual_q_and_p",w=20,h=9),
                   list(p=dap$bar,n="barplot_top_da",w=10,h=9),
                   list(p=dap$panel,n="da_volcano_bar_panel",w=20,h=8)))
      if(!is.null(it$p)) safe_export({save_gg(it$p,file.path(dd,it$n),it$w,it$h);ec$plots<-ec$plots+1},it$n)
  }
  
  # ── TABELAS ──
  td<-file.path(output_root,ds,"tables")
  if(!is.null(alpha_stats_all[[ds]])) safe_export({
    wa<-createWorkbook()
  ...
  if(!is.null(beta_stats_all[[ds]])) safe_export({
    wb<-createWorkbook()
  ...
  if(!is.null(da_results_all[[ds]])) safe_export({
    wd<-createWorkbook()
    add_fmt_sheet(wd,"All_Results",da_results_all[[ds]],"#833C00")
    sig_q<-da_results_all[[ds]][da_results_all[[ds]]$q_value<q_threshold,]
    if(nrow(sig_q)>0) add_fmt_sheet(wd,"Significant_q",sig_q,"#833C00")
    sig_p<-da_results_all[[ds]][da_results_all[[ds]]$p_value<0.05,]
    if(nrow(sig_p)>0) add_fmt_sheet(wd,"Significant_p",sig_p,"#833C00")
...
safe_export({
  ...
  if(!is.null(all_da_sig_q)&&nrow(all_da_sig_q)>0) add_fmt_sheet(wm,"DA_Sig_qvalue",all_da_sig_q,"#833C00")
  if(!is.null(all_da_sig_p)&&nrow(all_da_sig_p)>0) add_fmt_sheet(wm,"DA_Sig_pvalue",all_da_sig_p,"#833C00")
...
# ── SESSION INFO ──
tryCatch({sink(file.path(output_root,"session_info.txt"))
  ...
  print(sessionInfo());sink()},error=function(e){tryCatch(sink(),error=function(e2)NULL)})
...
expected<-c("pca_PC1_vs_PC2","pca_PC1_vs_PC3","sequencing_depth","alpha_all_metrics",
            ...
            "nmds_panel","heatmap_top_features","volcano_qvalue","volcano_pvalue",
            "volcano_dual_q_and_p","barplot_top_da","da_volcano_bar_panel",
            "_alpha_diversity.xlsx","_beta_diversity.xlsx","_differential_abundance.xlsx")

for(ds in names(datasets_grouped_list)){
  dp2<-file.path(output_root,ds); af<-list.files(dp2,recursive=TRUE)
  npng<-sum(grepl("\\.png$",af)); nxl<-sum(grepl("\\.xlsx$",af))
...
if(dir.exists(cp)){cat("  [consolidated]\n");for(f in list.files(cp)) cat(sprintf("    %s\n",f))}
